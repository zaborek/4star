...
filename1 = "TAP001_20200211.TXT"
filename2 = "/Users/zaborek/private/projects/4star/oldtap/TAP001.TXT"
clean_tap = function(filename){
  ...
  # location eq 1,61,62, -1
  idx.eq1 = which(dat$loc %in% c(-1, 1, 61,61))
  dat = dat[idx.eq1,]
...
new = clean_tap(filename1)
old = clean_tap(filename2)

idx.keep.new = new$barcode %in% old$barcode
new = new[idx.keep.new,]


idx.keep.old = old$barcode %in% new$barcode
old = old[idx.keep.old,]

table(old$barcode == new$barcode)

out = cbind(old$location, new$location, ifelse(old$location==new$location, 1,-1))
